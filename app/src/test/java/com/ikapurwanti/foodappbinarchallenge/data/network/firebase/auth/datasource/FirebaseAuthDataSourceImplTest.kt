...
class FirebaseAuthDataSourceImplTest {
    ...
    @Test
    fun `test register`() {
        coEvery { firebaseAuth.createUserWithEmailAndPassword(any(), any()) } returns mockTaskAuthResult()
        coEvery { firebaseAuth.createUserWithEmailAndPassword(any(), any()).result.user?.updateProfile(any()) } returns mockTaskVoid()
        runTest {
